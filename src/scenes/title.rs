...
impl GameScene for TitleScene {
    ...
    fn update(&mut self) -> Result<SceneTransition, i32> {
        ...
        {
            self.hold_down_tick = match self.hold_down_tick.checked_sub(crate::ONE_FRAME.as_millis())
            {
                Some(v) => v,
                None => 0
            };
    ...
    fn draw(&mut self) {
        ...
        crate::core::utils::draw_text_center("Press `Space` to start", 24, screen_height() * 0.6);

        let percentage = self.hold_down_tick as f32 / 500.0;
        let pos_y = screen_height().lerp( screen_height() * 0.7,percentage);
